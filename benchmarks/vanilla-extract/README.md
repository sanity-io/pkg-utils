...
Last run: 2026-07-17 (after the `@sanity/vanilla-extract-vite-plugin` bundled-dev fix — the
compiler is no longer closed by the mid-serve `buildEnd` of `experimental.bundledDev`, a
lifecycle change with no expected steady-state cost, and none observed — and after scoping
vite's own rolldown copy back to the 1.1.5 it pins via the `vite>rolldown` override, while
the library-build side stays on rolldown 1.2.0), full default suite
(`pnpm benchmark:vanilla-extract`) on Node.js 24.18.0, Linux x64, Intel Xeon, **4 cores**;
Rollup 4.62.2, Rolldown 1.2.0 (library builds) / 1.1.5 (inside Vite), Vite 8.1.5, Vitest
4.1.10, yuku-parser 0.6.4. Values are mean wall-clock milliseconds from that runner and are
machine-specific — compare ratios, not absolute numbers. (Vite-side numbers moved a few
percent with the vite-internal rolldown now at 1.1.5; the library-build ratios held at the
1.2.0 level, 2.87–2.99x → 2.87–3.21x within run-to-run noise.)
...
| ------------------------ | ----------------------------------------: | ---------------------------------------------------: | ------------------- |
| No minify, no target     |                               1,127.00 ms |                                            386.09 ms | Sanity 2.92x faster |
| Minify                   |                               1,177.90 ms |                                            395.81 ms | Sanity 2.98x faster |
| Target chrome61          |                               1,270.46 ms |                                            395.28 ms | Sanity 3.21x faster |
| Minify + target chrome61 |                               1,157.21 ms |                                            394.89 ms | Sanity 2.93x faster |
| Debug identifiers        |                               1,452.58 ms |                                            446.22 ms | Sanity 3.26x faster |

Debug identifiers cost each pipeline `debug − baseline`: **+325.6 ms** for the official
babel-based transform, **+60.1 ms** for the Sanity `yuku-parser` pass — the transform runs
...
| ----------- | -----------------------------: | ------------------------------------: | ------------------- |
| Short       |                      969.66 ms |                             789.24 ms | Sanity 1.23x faster |
| Debug       |                    1,301.01 ms |                             866.66 ms | Sanity 1.50x faster |
...
| -----------------------------: | ------------------------------------: | ------------------- |
|                    4,602.12 ms |                           3,392.00 ms | Sanity 1.36x faster |

### Vite dev HMR (10 samples each)

| Scenario                               | Official plugin | Sanity plugin | Relative result       |
| -------------------------------------- | --------------: | ------------: | --------------------- |
| Single `.css.ts` leaf edit             |        22.04 ms |      21.60 ms | Sanity 1.02x faster   |
| Shared theme edit, 100 style importers |       160.97 ms |     173.92 ms | Official 1.08x faster |
...
| ----------------: | --------------: | ------------: | ------------------- |
|                 0 |       418.64 ms |     244.40 ms | Sanity 1.71x faster |
|             1,000 |       441.18 ms |     252.99 ms | Sanity 1.74x faster |
|             5,000 |       633.63 ms |     402.94 ms | Sanity 1.57x faster |
